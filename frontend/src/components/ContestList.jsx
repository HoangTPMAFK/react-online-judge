...
function ContestList({ contests }) {
    ...
    const [filteredContests, setFilteredContests] = useState(contests.slice(1, contestsPerPage));
    ...
    const search = () => {
        const filter = contests.filter(c => c.title.toLowerCase().includes(searchStr.current.value.toLowerCase()));
    ...
    const searchByStatus = () => {
        ...
        if (statusValue === "All") {
            ...
            const filter = contests.filter(c => c.status.toLowerCase().includes(statusValue.toLowerCase()));
            setFilteredContests(filter);
            setCurrentPage(1);
        }
    }
        ...
        <div className="p-4">
            ...
            <ul className="my-4">
                {filteredContests.map(contest => (
                    <li key={contest.no} className="flex flex-row justify-between items-center px-2 py-2 border hover:cursor-pointer" onClick={() => window.location.href = `/contest/${contest.no}`}>
                        <div className="flex flex-row items-center">
                            ...
                            <div>
                                ...
                                <div className="flex flex-row items-center">
                                    ...
                                    <div className="text-sm text-gray-500">{contest.date} • {contest.duration}</div>
                                </div>
                            </div>
                        </div>
                        <div className={"border rounded-xl flex items-center px-3 " + statusBadge[contest.status]}>
