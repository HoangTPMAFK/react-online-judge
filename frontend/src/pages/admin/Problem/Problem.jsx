...
function Problem() {
    ...
    async function fetchData() {
      try {
        const response = await fetch(
          "http://localhost:8080/contest-programing/api/problem/"
        );
        if (!response.ok) {
      ...
      selector: (row) => (
        <div>
          <button
            ...
            onClick={() => {
              window.location.href = "/admin/problem/" + row.id;
            }}
          >
            View
          </button>
          <button
            ...
            onClick={() => {
              window.location.href = "/admin/problem/" + row.id + "/edit";
            }}
          >
            Edit
          </button>
  ...
  const handleSearch = () => {
    ...
    const filteredContests = data.filter((data) =>
